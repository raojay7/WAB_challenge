...
from make_dataloader import make_dataloader,make_dataloader_without_crop
from dataset import inst_process

DEBUG = False
DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

def infer_vid(metric_net, bbox_json, bbox_scale='SM', key='feat', net_type='effnet',crop=True):
    ...
    metric_net.eval()
    if crop:
        metric_img_ldr = tqdm(make_dataloader(inst_ds, 32, bbox_scale, net_type))
    else:
        metric_img_ldr = tqdm(make_dataloader_without_crop(inst_ds, 32, bbox_scale, net_type,crop=crop))
    feat_list = []
    idx_list = []
    with torch.no_grad():
...
def get_pred_vid(metric_net,net_type, bbox_json, save_path, key ,crop=True):
    ...
    inst_ds = infer_vid(metric_net, bbox_json, bbox_scale='SM', key=key,net_type=net_type,crop=crop)
