...
metric_net_b5a = ENet(num_classes=59895, feat_dim=512, cos_layer=True, xbm=None, dropout=0., m=0.30, pool='gem_freeze', image_net='tf_efficientnet_b5_ns', pretrained=False)
...
# pred_img = get_pred_img(metric_net_b5a,'effnet', '../../bbox_sub/myspace/test_img_bbox_16.json', '../myspace/output/ensemble/pred_img_test_b5a.json', 'feat_b5a')
pred_img = get_pred_img(metric_net_b5a,'effnet', './yolo_master/myspace/test_img_bbox_16.json', './metric_sub/myspace/output/ensemble/pred_img_test_b5a.json', 'feat_b5a')
# pred_vid = get_pred_vid(metric_net_b5a, 'effnet', '../../bbox_sub/myspace/test_vid_bbox_16.json', '../myspace/output/ensemble/pred_vid_test_b5a.json', 'feat_b5a')
pred_vid = get_pred_vid(metric_net_b5a, 'effnet', './yolo_master/myspace/test_vid_bbox_16.json', './metric_sub/myspace/output/ensemble/pred_vid_test_b5a.json', 'feat_b5a')
...
pred_img = get_pred_img(metric_net_deit,'vit_224', './metric_sub/myspace/output/ensemble/pred_img_test_b5a.json', './metric_sub/myspace/output/ensemble/pred_img_test_b5a_deit.json', 'feat_deit')
...
pred_vid = get_pred_vid(metric_net_deit, 'vit_224', './metric_sub/myspace/output/ensemble/pred_vid_test_b5a.json', './metric_sub/myspace/output/ensemble/pred_vid_test_b5a_deit.json', 'feat_deit')
...
for (k, v) in checkpoint['model_state_dict'].items():
    new_k = k[7:]
    if new_k=='arcface.loss.weight':
        new_k='arcface2.loss.weight'
    new_state_dict[new_k] = v
metric_net_b6.load_state_dict(new_state_dict)
...
pred_img = get_pred_img(metric_net_b6,'effnet', './metric_sub/myspace/output/ensemble/pred_img_test_b5a_deit.json', './metric_sub/myspace/output/ensemble/pred_img_test_b5a_deit_b6.json', 'feat_b6')
...
pred_vid = get_pred_vid(metric_net_b6, 'effnet','./metric_sub/myspace/output/ensemble/pred_vid_test_b5a_deit.json', './metric_sub/myspace/output/ensemble/pred_vid_test_b5a_deit_b6.json', 'feat_b6')
...
##将所有待融合的模型特征stack到一起
pred_img_df['feat'] = normalize(np.stack(pred_img_df['feat_b5a']+pred_img_df['feat_deit']+pred_img_df['feat_b6'])).tolist()
pred_img_df = pred_img_df.drop(columns=['feat_b5a','feat_deit','feat_b6'])
pred_vid_df['feat'] = normalize(np.stack(pred_vid_df['feat_b5a'] +pred_vid_df['feat_deit']+pred_vid_df['feat_b6'])).tolist()
pred_vid_df = pred_vid_df.drop(columns=['feat_b5a','feat_deit','feat_b6'])
...
end_time=time.time()
print('compute_distmat cost time {}/s'.format(end_time-start_time))
