...
from make_dataloader import make_dataloader_without_crop
...
class Config:
    def __init__(self):
        ...
        self.STAGE_EPOCHS = 1#10
        self.BATCH_SIZE = 64#32
        self.INFER_BATCH_SIZE = 32#32
        self.LOG_INTERVAL = 100#200
        self.SCALE = 'SM'
        self.OUTPUT_PATH = './metric_sub/myspace/output/arcface_b5a'
...
def main(cfg):
    ...
    train_ldr = make_dataloader_without_crop(train_json_list, cfg.BATCH_SIZE, cfg.SCALE, 'train',cfg.NUM_WORKERS,cfg.NET_TYPE,cfg.CROP)
    ...
    valid_img_ldr = make_dataloader_without_crop(valid_img_json_list, cfg.INFER_BATCH_SIZE, cfg.SCALE, 'valid',cfg.NUM_WORKERS,cfg.NET_TYPE,cfg.CROP)
    valid_vid_ldr = make_dataloader_without_crop(valid_vid_json_list, cfg.INFER_BATCH_SIZE, cfg.SCALE, 'valid',cfg.NUM_WORKERS,cfg.NET_TYPE,cfg.CROP)
    ...
    if cfg.RESUME:
        ...
        ##修改
        #从pretrain.pt加载数据至分布式训练，增加(.module)
...
if __name__ == "__main__":
    ...
    cfg = Config()
    cfg.PRETRAINED_FILE = './metric_sub/pretrained/pretrained_effnet_b5.pt'
    main(cfg)
