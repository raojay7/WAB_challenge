...
def infer_img(metric_net, bbox_json, bbox_scale='SM', key='feat',net_type='effnet',crop=True):
    ...
    metric_net.eval()
    if crop:
        metric_img_ldr = tqdm(make_dataloader(inst_ds, 32, 'SM',net_type))
    else:
        metric_img_ldr = tqdm(make_dataloader_without_crop(inst_ds, 32, 'SM',net_type,crop=crop))
...
def get_pred_img(metric_net,net_type, bbox_json, save_path, key ,crop=True):
    ...
    inst_ds = infer_img(metric_net, bbox_json, bbox_scale='SM', key=key,net_type=net_type,crop=crop)
